...
function matchesFilter(row: Row, filter: Filter): boolean {
  ...
  return Object.entries(filter).every(
    ([key, value]) => compareValues(row[key], value) === 0,
  );
}
...
function createFixture() {
  ...
  const mateActivityRows: Row[] = [
    { mateId: 3, activityId: 2 },
    { mateId: 4, activityId: 2 },
  ];
  const userRows: Row[] = [
...
describe('BookingsService', () => {
  ...
  describe('create', () => {
    ...
    it('rejects booking yourself', async () => {
      await expect(fixture.service.create(5, dto)).rejects.toBeInstanceOf(
        BadRequestException,
      );
    });
    ...
    it('rejects when the activity is not listed on the mate profile', async () => {
      ...
      await expect(
        fixture.service.create(7, { ...dto, mateId: 9 }),
      ).rejects.toBeInstanceOf(UnprocessableEntityException);
    });

    it('rejects when startTime is not before endTime', async () => {
      await expect(
        fixture.service.create(7, {
          ...dto,
          startTime: '12:00',
          endTime: '10:00',
        }),
    ...
    it('rejects when the mate is unavailable at the requested time', async () => {
      fixture.mateAvailabilityService.isWithinAvailability.mockResolvedValueOnce(
        false,
      );
      await expect(fixture.service.create(7, dto)).rejects.toBeInstanceOf(
        UnprocessableEntityException,
    ...
    it('allows a non-overlapping booking for the same mate on the same day', async () => {
      await fixture.service.create(7, dto);
      const second = await fixture.service.create(8, {
        ...dto,
        startTime: '13:00',
        endTime: '14:00',
      });
  ...
  describe('lifecycle transitions', () => {
    ...
    it('is idempotent when accepting an already-confirmed booking', async () => {
      ...
      const again = await fixture.service.accept(
        { id: 5, role: 'mate' },
        created.id,
      );
    ...
    it('rejects accept from someone other than the mate owner', async () => {
      const created = await fixture.service.create(7, dto);
      await expect(
        fixture.service.accept({ id: 7, role: 'renter' }, created.id),
    ...
    it('lets the mate owner decline a pending booking and notifies the renter', async () => {
      ...
      const declined = await fixture.service.decline(
        { id: 5, role: 'mate' },
        created.id,
      );
    ...
    it('rejects accepting a booking that is not pending', async () => {
      ...
      await expect(
        fixture.service.accept({ id: 5, role: 'mate' }, created.id),
      ).rejects.toBeInstanceOf(UnprocessableEntityException);
    });
    ...
    it('lets an admin cancel and notifies both participants', async () => {
      ...
      const recipients = fixture.notificationRows
        .map((row) => row.userId)
        .sort();
    ...
    it('rejects cancel from someone who is not a participant or admin', async () => {
      const created = await fixture.service.create(7, dto);
      await expect(
        fixture.service.cancel({ id: 8, role: 'renter' }, created.id),
      ).rejects.toBeInstanceOf(ForbiddenException);
    });
    ...
    it('lets the mate owner complete a confirmed booking after its end time and notifies the renter', async () => {
      const created = await fixture.service.create(7, dto);
      await fixture.service.accept({ id: 5, role: 'mate' }, created.id);
      await fixture.database.orm.public.Booking.where({
        id: created.id,
      }).update({
    ...
    it('rejects completing a confirmed booking before its end time', async () => {
      ...
      await fixture.service.accept({ id: 5, role: 'mate' }, created.id);
      await expect(
        fixture.service.complete({ id: 5, role: 'mate' }, created.id),
      ).rejects.toBeInstanceOf(UnprocessableEntityException);
    });
    ...
    it('rejects cancelling a booking after its start time', async () => {
      ...
      await expect(
        fixture.service.cancel({ id: 7, role: 'renter' }, created.id),
      ).rejects.toBeInstanceOf(UnprocessableEntityException);
    });
  });
  ...
  describe('findMine / findOne', () => {
    ...
    it('lists only the current renter own bookings', async () => {
      await fixture.service.create(7, dto);
      await fixture.service.create(8, {
        ...dto,
        startTime: '13:00',
        endTime: '14:00',
      });

      const result = await fixture.service.findMine(
        { id: 7, role: 'renter' },
        {},
      );
    ...
    it('allows a participant to view booking detail', async () => {
      const created = await fixture.service.create(7, dto);
      const detail = await fixture.service.findOne(
        { id: 7, role: 'renter' },
        created.id,
      );
    ...
    it('rejects viewing a booking detail for a non-participant, non-admin caller', async () => {
      const created = await fixture.service.create(7, dto);
      await expect(
        fixture.service.findOne({ id: 8, role: 'renter' }, created.id),
      ).rejects.toBeInstanceOf(NotFoundException);
    });
  });
});
