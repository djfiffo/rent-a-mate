...
export class PaymentsService {
   ...
   */
  async assertParticipant(user: AuthUser, bookingId: number): Promise<PaymentParticipants> {
  ...
  async pay(user: AuthUser, bookingId: number): Promise<CreatePaymentResult> {
    ...
    if (booking.renterId !== user.id) {
      throw new ForbiddenException('Only the renter who owns this booking can pay for it');
    }
    if (booking.status !== 'confirmed') {
      throw new UnprocessableEntityException('PAYMENT_NOT_ALLOWED');
    }

    const existing = await this.database.orm.public.Payment.where({ bookingId }).first();
    ...
    const amountInSmallestUnit = this.toSmallestCurrencyUnit(booking.totalPrice);
    ...
    if (existing?.status === 'pending' && existing.providerReference) {
      ...
      return { bookingId, status: existing.status, clientSecret: intent.client_secret };
    }
    ...
    return this.database.transaction(async (tx) => {
      ...
      return { bookingId, status: 'pending' as const, clientSecret: intent.client_secret };
    });
  }

  async confirmLocalMockPayment(user: AuthUser, bookingId: number): Promise<PaymentDetail> {
    if (!this.stripeProvider.localMockEnabled) {
    ...
    const payment = await this.database.orm.public.Payment.where({ bookingId }).first();
    if (
      !payment ||
      payment.status !== 'pending' ||
      payment.providerReference !== `local_mock_${bookingId}`
    ) {
      throw new UnprocessableEntityException('LOCAL_TEST_PAYMENT_NOT_AVAILABLE');
    }
   ...
   */
  async listMine(user: AuthUser, query: ListPaymentsQueryDto): Promise<PaginatedResult<PaymentDetail>> {
    ...
    const bookings = await this.database.orm.public.Booking.where(bookingFilter).all();
    if (bookings.length === 0) {
      return { items: [], meta: { page, limit, total: 0, totalPages: 0 } };
    }

    const bookingIds = bookings.map((booking) => booking.id);
    const payments = await this.database.orm.public.Payment.where((p) => p.bookingId.in(bookingIds)).all();
    const paymentMap = new Map(payments.map((payment) => [payment.bookingId, payment]));
    ...
    const items = sortedBookings
      ...
      .map((booking) => this.toDetail(booking, paymentMap.get(booking.id) ?? null));
  ...
  async refund(
    ...
    const payment = await writer.orm.public.Payment.where({ bookingId }).first();
    ...
    const updated = await writer.orm.public.Payment.where({ bookingId }).update({ status: 'refunding' });
    if (!updated) {
  ...
  private toDetail(booking: BookingRecordForPayment, payment: PaymentRecord | null): PaymentDetail {
    return {
      bookingId: booking.id,
      amount: booking.totalPrice,
  ...
  private toSmallestCurrencyUnit(amount: BookingRecordForPayment['totalPrice']): number {
