import { Controller, Get, Header, Inject, NotFoundException, Query, ServiceUnavailableException, StreamableFile } from '@nestjs/common';
...
const photoKey = /^mates\/[1-9]\d*\/[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}\.(jpg|png|gif|webp)$/i;
const mimeTypes = { jpg: 'image/jpeg', png: 'image/png', gif: 'image/gif', webp: 'image/webp' } as const;
...
export class MateUploadsController {
  ...
  async getPhoto(@Query('key') key: string): Promise<StreamableFile> {
    ...
    try {
      ...
      return new StreamableFile(stream, { type: mimeTypes[match[1].toLowerCase() as keyof typeof mimeTypes] });
    } catch (error) {
      const code = (error as { code?: string }).code;
      if (code === 'NoSuchKey' || code === 'NotFound') throw new NotFoundException('Photo not found');
