...
describe('MateAvailabilityService', () => {
  it('subtracts a pending booking and splits a weekly window', async () => {
    const weekly = [{ id: 1, mateId: 11, dayOfWeek: 6, startTime: '10:00', endTime: '18:00', createdAt: null, updatedAt: null }];
    const booking = {
      id: 3,
      mateId: 11,
    ...
    await expect(service.getPublicAvailability(11, '2099-01-10')).resolves.toEqual([
  ...
  it('replaces the full schedule transactionally and rejects overlap before deletion', async () => {
    ...
    const create = vi.fn(async (value: unknown) => { created.push(value); return value; });
    const mateAvailability = { where: vi.fn().mockReturnValue({ deleteAndCount, all: vi.fn().mockResolvedValue(created) }), create };
    const database = {
    ...
    await service.replace(7, { availability: [
    ...
    await expect(service.replace(7, { availability: [
      { dayOfWeek: 1, startTime: '09:00', endTime: '12:00' },
  ...
  it('rejects public availability dates in the past', async () => {
    ...
    await expect(service.getPublicAvailability(11, '2000-01-01')).rejects.toMatchObject({ status: 422 });
  });
});
