...
export class MateLookupController {
  ...
  async activities() {
    return successResponse('OK', {
      items: await this.lookupService.activities(),
    });
  }
  ...
  async interests() {
    return successResponse('OK', {
      items: await this.lookupService.interests(),
    });
  }
  ...
  async provinces() {
    return successResponse('OK', {
      items: await this.lookupService.provinces(),
    });
  }
  ...
  async districts(@Param('provinceId', ParseIntPipe) provinceId: number) {
    return successResponse('OK', {
      items: await this.lookupService.districts(provinceId),
    });
  }
}
