...
const errorsFor = async (query: object) => validate(plainToInstance(ListMatesQueryDto, query));
...
describe('ListMatesQueryDto', () => {
  ...
  it('rejects a limit greater than 100 and an unknown sort', async () => {
    ...
    expect(errors.map((error) => error.property)).toEqual(expect.arrayContaining(['limit', 'sort']));
  });
});
