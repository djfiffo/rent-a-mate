...
describe('Payments, webhooks, reports, and admin APIs (e2e)', () => {
  ...
  it('PAY-01/02/03/09 creates one intent, resumes it, and exposes participant status', async () => {
    const booking = await directBooking(data, {
      status: 'confirmed',
      daysFromNow: 2,
      totalPrice: '750.25',
    });
    ...
    expect(
      await db.orm.public.Payment.where({ bookingId: booking.id }).first(),
    ).toBeNull();
  ...
  it('PAY-05/06/07 enforces booking state, renter ownership, and refund state', async () => {
    const pending = await directBooking(data, {
      status: 'pending',
      daysFromNow: 2,
    });
    ...
    const confirmed = await directBooking(data, {
      status: 'confirmed',
      daysFromNow: 3,
    });
  ...
  it('PAY-04/10/11 and HOOK-01/05 finalize payment once and list it for participants', async () => {
    ...
    const history = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(history.body.data.items).toContainEqual(
      expect.objectContaining({ bookingId: booking.id, status: 'paid' }),
    );
    await request(app.getHttpServer())
      .get(path)
      .set(bearer(tokens.renter2))
    ...
    expect(
      await db.orm.public.StripeWebhookEvent.where({ id: event.id }).all(),
    ).toHaveLength(1);
  });

  it('HOOK-02/03 dispatches payment failure and refund events', async () => {
    const failedBooking = await directBooking(data, {
      status: 'confirmed',
      daysFromNow: 2,
    });
    ...
    expect(
      (await db.orm.public.Payment.where({ id: failedPayment.id }).first())
        ?.status,
    ).toBe('failed');

    const refundedBooking = await directBooking(data, {
      status: 'cancelled',
      daysFromNow: 3,
    });
    ...
    await sendWebhook(app, {
      ...
      data: {
        object: {
          id: 'ch_e2e',
          payment_intent: refundedPayment.providerReference,
        },
      },
    }).expect(200);
    expect(
      (await db.orm.public.Payment.where({ id: refundedPayment.id }).first())
        ?.status,
    ).toBe('refunded');
  });
  ...
  it('HOOK-04/07 rejects bad signatures and safely records ignored events', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(400);
    expect(
      await db.orm.public.StripeWebhookEvent.where({ id: event.id }).first(),
    ).toBeNull();

    await sendWebhook(app, { ...event, id: 'evt_ignored' }).expect(200);
    expect(
      await db.orm.public.StripeWebhookEvent.where({
        id: 'evt_ignored',
      }).first(),
    ).not.toBeNull();
  });
  ...
  it('ADMIN-04/05/06 bans a user, revokes every refresh session, and unbans without restoring sessions', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(200)
      .expect(({ body }) =>
        expect(body.data.user).toMatchObject({ isBanned: true }),
      );
  ...
  it('ADMIN-07/08 activates users and verifies only mate accounts', async () => {
    ...
    await db.orm.public.Mate.where({ id: data.mate1.id }).update({
      isActive: false,
    });
    await request(app.getHttpServer())
      ...
      .expect(200);
    expect(
      (await db.orm.public.Mate.where({ id: data.mate1.id }).first())?.isActive,
    ).toBe(true);
  ...
  it('ADMIN-10 lists and filters all bookings independently of participation', async () => {
    ...
    expect(response.body.data.items).toContainEqual(
      expect.objectContaining({ id: booking.id, status: 'confirmed' }),
    );
  });
  ...
  it('REPORT-01/03/06/07/08 creates related reports and lets admins resolve them', async () => {
    ...
    expect(list.body.data.items).toContainEqual(
      expect.objectContaining({ id: created.body.data.report.id }),
    );
    await request(app.getHttpServer())
  ...
  it('ANALYTICS-01/03/04/05/06 aggregates daily booking, paid revenue, and active users', async () => {
    ...
    await directBooking(data, {
      status: 'completed',
      daysFromNow: 0,
      startTime: '00:00',
      endTime: '01:00',
    });
    const paidBooking = await directBooking(data, {
      status: 'confirmed',
      daysFromNow: 0,
      startTime: '02:00',
      endTime: '03:00',
    ...
    expect(response.body.data.bookingCounts.completed).toBeGreaterThanOrEqual(
      1,
    );
    expect(response.body.data.bookingCounts.confirmed).toBeGreaterThanOrEqual(
      1,
    );
    expect(response.body.data.paidRevenue).toBe(125.5);
    expect(response.body.data.daily).toHaveLength(1);
    expect(response.body.data.daily[0]).toMatchObject({
      date: today.toString(),
      paidRevenue: 125.5,
    });
  });
