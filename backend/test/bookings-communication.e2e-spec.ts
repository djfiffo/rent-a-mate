...
describe('Bookings, reviews, REST messages, and notifications (e2e)', () => {
  ...
  it('BOOK-03/04/05/06/08 rejects invalid DTOs, state, ownership, and availability', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .send({ ...valid, endTime: '10:30' })
      .expect(422);
    await request(app.getHttpServer())
      .post('/api/v1/bookings')
      .set(bearer(tokens.renter1))
      .send({ ...valid, startTime: '11:00', endTime: '10:00' })
      .expect(400);
    await request(app.getHttpServer())
      .post('/api/v1/bookings')
      .set(bearer(tokens.renter1))
      .send({ ...valid, date: 'invalid' })
      .expect(400);
    await request(app.getHttpServer())
      .post('/api/v1/bookings')
      .set(bearer(tokens.renter1))
      .send({ ...valid, totalPrice: 1 })
      .expect(400);
    await request(app.getHttpServer())
      .post('/api/v1/bookings')
      .set(bearer(tokens.renter1))
      .send({ ...valid, mateId: 999999 })
      .expect(404);
    await request(app.getHttpServer())
      .post('/api/v1/bookings')
      .set(bearer(tokens.renter1))
      .send({ ...valid, activityId: data.activity2.id })
      .expect(422);
    await request(app.getHttpServer())
      .post('/api/v1/bookings')
      .set(bearer(tokens.renter1))
      .send({ ...valid, startTime: '18:00', endTime: '19:00' })
      .expect(422);
    await request(app.getHttpServer())
      .post('/api/v1/bookings')
  ...
  it('BOOK-09/10 rejects blocking overlaps and allows adjacent or cancelled slots', async () => {
    const date = getFutureDate(2);
    await createBooking(app, tokens.renter1, data, {
      date,
      startTime: '10:00',
      endTime: '11:30',
    }).expect(201);
    await createBooking(app, tokens.renter2, data, {
      date,
      startTime: '10:30',
      endTime: '12:00',
    }).expect(409);
    const adjacent = await createBooking(app, tokens.renter2, data, {
      date,
      startTime: '11:30',
      endTime: '12:30',
    }).expect(201);
  ...
  it('BOOK-11/12/13 lists only participant bookings and protects detail from IDOR', async () => {
    ...
    const renterList = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(renterList.body.data).toMatchObject({
  ...
  it('BOOK-14/16/17 accepts idempotently and enforces mate ownership and transition rules', async () => {
    ...
    expect(
      notifications.filter((item) => item.type === 'booking_confirmed'),
    ).toHaveLength(1);
  });
  ...
  it('BOOK-15 declines idempotently and releases the slot', async () => {
    ...
    expect(
      notifications.filter((item) => item.type === 'booking_declined'),
    ).toHaveLength(1);
  });
  ...
  it('BOOK-18/19 cancels by participants or admin, notifies correct recipients, and is idempotent', async () => {
    ...
    expect(
      mateNotifications.filter((item) => item.type === 'booking_cancelled'),
    ).toHaveLength(1);
    ...
    await request(app.getHttpServer())
      ...
      .set(bearer(tokens.admin))
      .expect(200);
    const adminCancellation = await db.orm.public.Notification.where({
      bookingId: adminBooking.body.data.id,
  ...
  it('BOOK-20/21 completes only confirmed past bookings as mate owner or admin', async () => {
    const completed = await directBooking(data, {
      status: 'confirmed',
      daysFromNow: -2,
    });
    await request(app.getHttpServer())
    ...
    const future = await directBooking(data, {
      status: 'confirmed',
      daysFromNow: 2,
      startTime: '14:00',
      endTime: '15:00',
  ...
  it('REVIEW-01/03/05 creates one review only after completion and embeds it in booking detail', async () => {
    ...
    const detail = await request(app.getHttpServer())
      ...
      .set(bearer(tokens.renter1))
      .expect(200);
    expect(detail.body.data.review).toMatchObject({
  ...
  it('REVIEW-06/07/08/10 updates, lists, and deletes reviews with ownership rules', async () => {
    ...
    const publicReviews = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(publicReviews.body.data).toMatchObject({
      averageRating: 4,
      reviewCount: 1,
    });
  ...
  it('MSG-01/02/03/04/05/06 persists participant messages and notifications with state gating', async () => {
    ...
    const list = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(list.body.data.items.map((item: { id: number }) => item.id)).toEqual(
      [sent.body.data.id],
    );
    await request(app.getHttpServer())
      .get(path)
  ...
  it('NOTIFY-01/02/03/04/05 lists, filters, and marks only owned notifications', async () => {
    ...
    const unread = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(
      unread.body.data.notifications.map((item: { id: number }) => item.id),
    ).toEqual([ownUnread.id]);
    expect(
      unread.body.data.notifications.map((item: { id: number }) => item.id),
    ).not.toContain(ownRead.id);
